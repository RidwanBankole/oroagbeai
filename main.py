...
def create_app():
    ...
    # ── USSD — always registered, no special packages needed ─────────────
    app.register_blueprint(ussd_bp)
    logger.info("USSD blueprint registered → /ussd/session (always active)")

    # ── IVR — registered only if ivr_handler imports successfully ────────
    # If transformers/torch/scipy/pydub are not installed, the import will
    # fail gracefully here and USSD will continue working perfectly.
    # Africa's Talking routes USSD calls to /ussd/session and voice calls
    # to /ivr/session — they are completely separate and never interfere.
    try:
        from app.ivr_handler import ivr_bp
        app.register_blueprint(ivr_bp)
        logger.info("IVR blueprint registered → /ivr/session (voice calls active)")
    except ImportError as e:
        logger.warning(
            "IVR blueprint not loaded (%s). "
            "Voice calls will not work but USSD is unaffected. "
            "To enable IVR install: transformers torch scipy pydub "
            "and apt-get install ffmpeg", e
        )
    except Exception as e:
        logger.warning(
            "IVR blueprint failed to register: %s. "
            "USSD is unaffected.", e
        )
    ...
    def health():
        return {
            "status":  "ok",
            "project": "Oro Agbe - Farmer's Matter",
        }, 200
    ...
    def pipeline():
        ...
        # Step 4 — TTS (non-fatal if packages missing)
        audio_url = None
        try:
            ...
            logger.warning("TTS packages not installed — audio_url will be null in response")
        except Exception as e:
