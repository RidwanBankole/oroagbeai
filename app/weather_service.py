...
def _pick_hour_value(hourly: Dict[str, List[Any]], field: str, current_time: str, default: float = 0.0) -> float:
    """
...
def get_weather(lat: float, lon: float, location_name: str) -> Optional[WeatherData]:
    ...
    Design choices:
    - Use `current` for actual real-time conditions
    - Use `hourly` only for fields not available in `current`, matched to current.time
    - Use `daily` for day summaries and tomorrow outlook
    """
    params = {
        "latitude": lat,
        "longitude": lon,
    ...
    try:
        ...
        tomorrow = {k: v[1] for k, v in daily.items()} if len(daily["weather_code"]) > 1 else today

        current_time = current["time"]
        weather_code_now = current["weather_code"]
        weather_condition_now = WMO_CODES.get(weather_code_now, "Variable conditions")
        weather_condition_tomorrow = WMO_CODES.get(tomorrow["weather_code"], "Variable conditions")
...
def weather_to_english_text(w: WeatherData) -> str:
    """
    Render the weather in a cleaner, more useful style.
    """
    return (
